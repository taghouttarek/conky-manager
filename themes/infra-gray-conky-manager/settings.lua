...
function draw_infra_widget(cr, x, y)
    ...
    -- Fetch data
    local raw = conky_parse("${exec python3 ~/.config/conky/infra-gray-conky-manager/fetch_infra_vulns.py --get_list --count 5 2>/dev/null}")
...
function draw_function(cr)
    ...
    local widget_x = positions["infra-gray-conky-manager"].x
    local widget_y = positions["infra-gray-conky-manager"].y
