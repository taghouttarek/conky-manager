...
r_accent, g_accent, b_accent = hex2rgb(HTML_accent)
...
function draw_function(cr)
    ...
    if enabled_kev then
        ...
        draw_vuln_widget(cr, kx2, ky2, "KEV", os.getenv("HOME") .. "/.config/conky/all-widgets-glass-conky-manager/fetch_vulns.py", r_crit_kev, g_crit_kev, b_crit_kev)
    end
    ...
    if enabled_infra then
        ...
        draw_vuln_widget(cr, ix, iy, "INFRA", os.getenv("HOME") .. "/.config/conky/all-widgets-glass-conky-manager/fetch_infra_vulns.py", r_crit_infra, g_crit_infra, b_crit_infra)
    end
...
function conky_start_widgets()
    local function draw_conky_function(cr)
        ...
        if not ok then
            local f = io.open(os.getenv("HOME") .. "/.config/conky/all-widgets-glass-conky-manager/error.log", "a")
            if f then
